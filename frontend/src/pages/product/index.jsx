      ...
      <div className={`bg-white/[0.03] backdrop-blur-md border ${status ? 'border-white/[0.08]' : 'border-rose-500/20 opacity-60'} rounded-[1.5rem] p-5 shadow-2xl transition-all`}>
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h4 className="text-sm font-black text-white tracking-tight uppercase truncate">{p.name}</h4>
            <div className="text-[9px] font-black text-white/30 truncate mt-1">{categories.find(c=>c.id == p.cat_id)?.name || "VOID"}</div>
            <div className="mt-3 flex items-center justify-between">
               <span className="text-base font-black text-yellow-400">{formatGBP(p.price)}</span>
               <div className="flex items-center gap-2">
                  <button onClick={() => { setForm({ ...p, oldImage: p.image, image: null }); setShowModal(true); }} className="p-2 bg-white/5 border border-white/10 rounded-xl text-yellow-400"><Edit size={14} /></button>
            ...
            <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-10 py-6 sm:py-8">
              ...
              <div className="mb-8 space-y-8">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight drop-shadow-lg uppercase leading-none">Products</h1>
                      <p className="text-white/40 mt-1.5 text-[10px] sm:text-[11px] font-black uppercase tracking-[0.2em] leading-none">Manage your menu items and pricing</p>
                    </div>
                  </div>
                     ...
                     <div className="flex items-center gap-3 overflow-x-auto pb-2 -mx-2 px-2 scrollbar-hide">
                       <div className="relative group">
                          ...
                          <select
                            ...
                            className="pl-8 pr-4 py-3 bg-white/5 border border-white/[0.08] rounded-xl text-white text-[9px] font-black uppercase tracking-widest appearance-none focus:outline-none focus:border-yellow-400/40 transition-all"
                          >
                            <option value="all" className="bg-[#0b1a3d]">ALL CATEGORIES</option>
                            {categories.map((c) => <option key={c.id} value={c.id} className="bg-[#0b1a3d]">{c.name.toUpperCase()}</option>)}
                          </select>
                       </div>
                       ...
                       <button onClick={() => setShowSearchModal(true)} className="px-5 py-3 bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-400 hover:to-amber-400 text-[#071428] font-black uppercase tracking-widest text-[9px] rounded-xl shadow-xl transition-all flex items-center justify-center gap-2 whitespace-nowrap active:scale-95">
                          <Globe size={14} /> Search Global
                       </button>

                       <button 
                         onClick={() => { resetForm(); setShowModal(true); }}
                         className="px-8 py-3 bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-400 hover:to-amber-400 text-slate-900 font-black uppercase tracking-widest text-[9px] rounded-xl shadow-2xl transition-all flex items-center justify-center gap-2 whitespace-nowrap border border-white/10"
                        >
              ...
              <div className="bg-[#0b1a3d]/60 backdrop-blur-xl rounded-[1.5rem] sm:rounded-[2.5rem] shadow-2xl border border-white/[0.08] overflow-hidden mb-12">
                 <div className="px-8 py-6 border-b border-white/[0.08] bg-white/5 flex justify-between items-center sm:px-10">
                    <h3 className="text-[11px] font-black text-white tracking-[0.3em] uppercase flex items-center gap-3">
                       <GripVertical size={16} className="text-yellow-400" /> Product List
                    </h3>
                                            ...
                                            >
                                              ...
                                              <td className="px-8 py-5">
                                                 <div className="min-w-0">
                                                    <div className="text-sm font-black text-white tracking-tight uppercase leading-tight truncate max-w-xs">{p.name}</div>
                                                    <div className="text-[9px] font-black text-white/30 tracking-[0.2em] mt-1 uppercase truncate max-w-[200px]">
                                              ...
                                              <td className="px-8 py-5">
                                                 <button 
                                                    ...
                                                    className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border transition-all ${p.status === 1 ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-500'}`}
                                                 >
